...
def fixTitlePage(s):
    ...
    s = (s[:b] +
         r'\release{' + ver + r''' for Python Version 3.1+}
\author{Dr. Andrew N. Harrington, Loyola University Chicago}
'''
         + s[p:])
    return s

def removeExtraContents(s):    
...
##    input('\n !!!!! Starting fix in Python, Press return to continue....:  ')
    ...
    # s = s.replace(badline, '')
    # quoteindex = r'''\index{string!delimiters}\index{"}\index{'}'''
    # if quoteindex not in s:
    #     print(' ?? no quote index ??')
    # else:
    #     print('changing quote index')
    # input('Press return to continue...')
    # s = s.replace(quoteindex,
    #               r'''\index{string!delimiters}\index{"-quote}\index{'}''')
##    s = removeSearch(s)
